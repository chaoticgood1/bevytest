...
use image::{io::Reader, open, ImageBuffer, ImageFormat, Rgb, RgbImage};
...
pub fn create_skybox(
    ...
    info!("plugin.image {:?}", plugin.image);
    // let texture_handle = asset_server.load(plugin.image.as_str());
    // let texture_handle: Handle<Texture> = asset_server.load("sky1.png");
    // let texture_handle = asset_server.get_handle("sky.png");

    let texture_handle: Handle<Texture> = asset_server.load("sky1.png");
    // textures.
    // let ls = asset_server.get_load_state("sky.png");
    // info!("{:?}", ls);
    // let t: &Texture = textures.get(texture_handle).unwrap();
    // println!("Data {:?}", t.data);
...
pub fn create_skybox_new(
    ...
    let texture = match textures.get(texture_handle.clone()) {
        Some(i) => i,
        None => return,
    };

    let dy_img = ImageBuffer::from_raw(
        texture.size.width,
        texture.size.height,
        texture.data.clone(),
    )
    ...
    let rgb_image = match dy_img {
        Some(i) => i,
        None => return,
    };

    let mesh = get_mesh_new(&rgb_image.into_rgb8());
    info!("Mesh");
    if let Ok(m) = mesh {
        // Load image as a texture asset.
...
fn get_mesh_new(rgb_image: &RgbImage) -> Result<Mesh, ImageError> {
    let (fx, fy) = find_uv_new(rgb_image)?;
...
fn find_uv_new(rgb: &RgbImage) -> Result<(Vec<f32>, Vec<f32>), ImageError> {
    // let rgb: RgbImage = image_buffer.clone();
...
fn find_uv(image: &str) -> Result<(Vec<f32>, Vec<f32>), ImageError> {
    ...
    let rgb: RgbImage = open(&format!("assets/{}", image))
        // let rgb: RgbImage = open(&format!("{}", image))
