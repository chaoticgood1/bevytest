...
fn mouse_motion_system(
  ...
  mut mouse_left_event: EventReader<CursorLeft>,
  mut mouse_entered_event: EventReader<CursorEntered>,
  mut query: Query<(&mut FlyCamera, &mut Transform)>,

  mouse_button_input: Res<Input<MouseButton>>,
) {
  if mouse_left_event.iter().next().is_some() {
    state.cursor_left = true;
  }
  if mouse_entered_event.iter().next().is_some() {
    state.cursor_left = false;
  }
  ...
  if delta.length_squared() < 0.01 && !state.dragged {
    return;
  }
  ...
  for (mut options, mut transform) in query.iter_mut() {
    ...
    // info!("pitch: {}, yaw: {}", options.pitch, options.yaw);
